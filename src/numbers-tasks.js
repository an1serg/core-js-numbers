...
function roundToPowerOfTen(num, pow) {
  const rounded = Math.round(num * 0.1 ** pow) * 10 ** pow;
  return rounded;
}
...
function getFibonacciNumber(index) {
  const fibanacciArr = [0, 1];
  let i = 0;
  let flag = true;
  while (flag) {
    if (fibanacciArr.length <= index + 1) {
      fibanacciArr.push(fibanacciArr[i] + fibanacciArr[i + 1]);
      i += 1;
    } else {
      flag = false;
    }
  }
  return fibanacciArr[index];
}
...
function getSumOfDigits(num) {
  const myNumString = String(num);
  let sum = 0;
  for (let i = 0; i < myNumString.length; i += 1) {
    sum += Number(myNumString[i]);
  }
  return sum;
}
